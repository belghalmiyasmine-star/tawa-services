...
- [ ] **AUTH-04**: User session persists across browser refresh (NextAuth.js sessions)
- [ ] **AUTH-05**: User can log in via Google or Facebook (NextAuth.js social providers)
...
- [ ] **KYC-01**: Provider can upload CIN/passport photo for identity verification
- [ ] **KYC-02**: Provider can upload selfie for identity match
- [ ] **KYC-03**: Provider can upload proof of address document
...
- [ ] **BOOK-01**: Client can directly book fixed-price services by selecting a time slot
- [ ] **BOOK-02**: Client can send quote request for "sur devis" services (describe job, provider responds with price)
- [ ] **BOOK-03**: Provider can accept or reject direct bookings and quote requests
- [ ] **BOOK-04**: Provider has 48h to respond to quote requests before auto-expiry
- [ ] **BOOK-05**: Booking status flow: PENDING → ACCEPTED → IN_PROGRESS → COMPLETED (with REJECTED and CANCELLED alternatives)
...
- [ ] **PAY-02**: Escrow model: client pays → platform holds → service completed → provider paid minus 12% commission
- [ ] **PAY-03**: Provider earnings dashboard showing earnings, pending payments, commission breakdown
- [ ] **PAY-04**: Auto-generated invoices/receipts for completed transactions
- [ ] **PAY-05**: Monthly earnings statements for providers
- [ ] **PAY-06**: Provider withdrawal requests (minimum 50 TND)
- [ ] **PAY-07**: Tax summaries and 5-year document retention
- [ ] **PAY-08**: Payment service abstraction layer (pluggable for future Konnect integration)
...
- [ ] **NOTF-01**: In-app notifications for booking requests, acceptance/rejection, new messages, new reviews, payments received, profile approval
- [ ] **NOTF-02**: Email notifications for transactional events (bookings, password reset, verification)
